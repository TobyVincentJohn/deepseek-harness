...
Preset files publish one unlocalized `name` and `description`, which Web uses for every `user` row and unknown `system` row. For the four localized shipped ids (`standard`, `code`, `minimal`, and `cordis`), Web resolves both fields from its active locale only when the roster marks the row `system`; `builder` and an identically named `user` preset keep their file metadata.
