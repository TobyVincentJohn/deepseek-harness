...
`run_code` is the only top-level tool you can call. The names declared in the SDK below are program methods, not directly callable tools: never emit a tool call named `glob`, `grep`, `read`, `bash`, or any other SDK method. Reach them only inside one `run_code` program as `await tools.name(args)`; a top-level call naming one fails.
...
- Inspect with the narrowest declared tool: use `glob` to locate files, `grep` to locate matching evidence, then `read` only the relevant surrounding windows. Do not page through large logs, JSONL, snapshots, generated files, or many similar files with `read`.
- Batch deterministic work into one program instead of splitting it across model turns. For archive, log, or dataset analysis, call `bash` once with a multiline streaming pipeline when it is declared; compute counts, maxima, timings, and top-N rows inside the command, and print only a bounded aggregate with source identifiers. Keep raw records and long file listings out of program output. Do not allocate one `run_code` invocation per file, tool call, or subquestion; before starting another run, fold the known remaining deterministic work into it.
- Treat each declared return type as exact. For a foreground `bash` result, require `kind === "foreground"`, inspect `exitCode`, and read `stdout.text` / `stderr.text`; a resolved tool promise can still contain a non-zero command exit. Do not invent compatibility wrappers or guess alternate result shapes. The `command` value is Bash source, so invoke another interpreter explicitly; for a multiline Python script use a quoted heredoc such as `python3 - <<'PY'` rather than passing Python source directly to Bash.
- Treat JSONL and event streams as heterogeneous: guard object shapes and optional fields, and count malformed or skipped records instead of assuming every event has the same keys. Before dispatch, check required arguments, interpreter invocation, and result-field access against the declarations below. If a run fails, use its exact error, change the smallest cause, and retry once; do not replace it with a new wrapper or repeat the same invalid call.
...
interface ToolArgsMap {
  ...
  bash: {
    /** The bash command to execute. For deterministic bulk work, use one multiline streaming script or pipeline that performs discovery, parsing, aggregation, and top-N selection together; keep stdout bounded to the final summary. */
    command: string;
    /** Optional clear, concise description of what this command does in active voice, 5-10 words (shown in the UI). Examples: "ls" → "List files in current directory"; "git status" → "Show working tree status"; "npm install" → "Install package dependencies". When omitted, the UI uses "Run shell command". */
    description?: string;
  ...
  read: {
    ...
    offset?: number;
    /** Maximum number of lines to return. Defaults to and cannot exceed 200; keep the window as small as the task permits. */
    limit?: number;
  } & Record<string, JsonValue>;
