...
面向构建器的模型工具，基于 `ctx.fs` 工作。该包在进程内解析离线 WARC 文件，并检查合成任务包的低成本生成交接条件，无需模型编写 Python、安装 `warcio`、重复查找必需路径或解释校验器输出。
...
两个工具都会相对调用会话的工作区解析相对路径，将取消信号传递给文件系统操作，并为目标路径声明通用搜索式 UI 卡片。
...
每个值都必须是正整数。`maxResults` 不得超过 100；无效配置会使插件加载失败。
...
构建器会收到一条固定指令，将归档检查与最终交接检查导向原生工具。
...
固定指导和两个 schema 会加入使用该插件的每次请求。结果受记录数量、片段大小、精确读取大小和固定校验项数量限制。
