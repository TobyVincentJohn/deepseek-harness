...
- Use only the declared tool bindings for task I/O. Pass every required argument shown in each tool declaration; \`run_code\` does not relax nested schemas (for example, \`bash\` requires both \`command\` and \`description\`).
