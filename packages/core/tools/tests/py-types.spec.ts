...
describe('renderToolsSdkPy', () => {
  ...
  it('declares identifier tools as async methods and lists exotic/reserved names as subscript comments', () => {
    ...
    expect(text).toContain('`bash` requires `command`, while its UI-only `description` is optional')
    expect(text).toContain('use `glob` to locate files, `grep` to locate matching evidence')
    expect(text).toContain('Do not page through large logs, JSONL, snapshots')
    expect(text).toContain('call `bash` once with a multiline streaming pipeline')
    expect(text).toContain('print only a bounded aggregate with source identifiers')
    expect(text).toContain('require `kind == "foreground"`, inspect `exitCode`')
    expect(text).toContain('read `stdout["text"]` / `stderr["text"]`')
    expect(text).toContain("use a quoted heredoc such as `python3 - <<'PY'`")
    expect(text).toContain('Treat JSONL and event streams as heterogeneous')
    expect(text).toContain('change the smallest cause, and retry once')
