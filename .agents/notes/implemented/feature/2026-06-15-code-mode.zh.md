...
SDK 指示模型编写一个所加载运行时语言的异步函数体（默认可擦除 TypeScript；Python 运行时下为 Python `async` 函数体——见[语言分发 note](2026-07-31-code-mode-language-dispatch.zh.md)），通过 `await tools.name(args)` 调用工具、传入必填字段、在需要时捕获被拒绝的调用，并仅 return 或 log 应重新进入上下文的输出。它说明每次调用都从全新的内存状态开始，并指示模型把确定性工作批量放入一个程序中；如果有可用的脚本工具，则通过一次多行脚本调用完成这类工作，而不是把它拆分到多个模型轮次。归档与日志分析会流式处理输入并输出紧凑汇总；只读任务避开变更类工具和创建文件的命令。TypeScript flavor 要求任务 I/O 使用工具绑定而非 Node 模块或 `process` API，并明确由嵌套工具负责相对于会话工作区的路径解析。两种 flavor 用各自的原语陈述同一调度约定：相互独立的只读调用可以（MAY）在 `Promise.all`（TypeScript）或 `asyncio.gather`（Python）下重叠，有副作用的调用按提交顺序单独运行，有依赖的工作用 `await` 排序。声明前缀可能与原生 schema 一样大，尤其在 `'both'` 下，但对提供方缓存保持稳定。[操作指南 note](../bug-fix/2026-08-22-code-mode-operational-guidance.zh.md)持有相关失败证据以及强化该指南的根调用提醒。
